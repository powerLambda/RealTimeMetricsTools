...
# --- Main container
FROM eclipse-temurin:8u332-b09-jdk-alpine as main

COPY --from=maven-build /home/maven/work/target/demo-fraud-webapp.jar .
EXPOSE 5656

ENTRYPOINT ["java","-Djava.security.egd=file:/dev/./urandom","-Dspring.profiles.active=local","-jar","demo-fraud-webapp.jar"]
